...
<div class="container  d-flex justify-content-center mt-3">
  ...
  @endif
        ...
        <h5 class="w-100 samuraimart-favorite-item-text">{{App\Models\Product::find($fav->favoriteable_id)->name}}
        </h5>
        <h6 class="w-100 samuraimart-favorite-item-text">
          &yen;{{App\Models\Product::find($fav->favoriteable_id)->price}}</h6>
        </div>
      </div>
      </div>
      ...
      <form method="POST" action="{{route('carts.store')}}" class="m-3 align-items-end">
        ...
        <input type="hidden" name="carriage"
        value="{{App\Models\Product::find($fav->favoriteable_id)->carriage_flag}}">
